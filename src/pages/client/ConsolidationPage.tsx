...
const ConsolidationWorkflow: React.FC<{ onClose: () => void }> = ({
  onClose,
}) => {
  ...
  const totalSteps = 4;
  const CONSOLIDATION_FEE_PER_PACKAGE = 50; // 50 MAD per package (~$5)
  const MAX_FEE = 250; // 250 MAD cap (~$25)
  ...
  const calculateEstimatedDimensions = () => {
    ...
    // Estimate consolidated dimensions
  ...
  const calculateConsolidationFee = () => {
    const fee = selectedPackages.length * CONSOLIDATION_FEE_PER_PACKAGE;
    return Math.min(fee, MAX_FEE);
  };

  const calculateSavings = () => {
    // Estimate savings: shipping 3 packages separately vs consolidated
    // Typical savings: 60-80%
    if (selectedPackages.length < 2) return 0;

    const separateShippingEstimate = selectedPackages.length * 350; // ~350 MAD per package
    const consolidatedShippingEstimate = 450; // One consolidated shipment
  ...
  const canProceed = () => {
    switch (currentStep) {
      case 1:
        return selectedPackages.length >= 2; // Need at least 2 packages
      case 2:
        return true;
      case 3:
        return true;
  ...
  const Step3Review = () => {
    const protectionFee = addProtection ? 25 : 0;
    const totalFee = calculateConsolidationFee() + protectionFee;
          ...
          <div className='space-y-3 text-sm'>
            ...
            <div className='border-t border-slate-200 pt-3 flex justify-between'>
              ...
              <span className='font-bold text-green-600'>
                {calculateConsolidationFee() + (addProtection ? 25 : 0)} MAD
              </span>
            </div>
          </div>
        </div>
  ...
  return (
    <div className='min-h-screen bg-gradient-to-br from-slate-900 to-green-900 flex items-center justify-center p-6'>
      {!showWorkflow ? (
        ...
        <AnimatePresence>
          <ConsolidationWorkflow onClose={() => setShowWorkflow(false)} />
        </AnimatePresence>
      )}
    </div>
  );
