// server/src/controllers/admin/adminDashboardController.ts - UPDATED WITH PHOTO REQUESTS
...
import { PhotoRequest } from '../../models/PhotoRequest.js'; // 🔥 NEW
...
export const getDashboardStats = async (
  ...
  try {
    ...
    const [
      ...
      // 🔥 NEW: Photo request stats
      pendingPhotoRequests,
      processingPhotoRequests,
      ...
      // 🔥 NEW: Photo Requests
      PhotoRequest.countDocuments({ status: 'pending' }),
      PhotoRequest.countDocuments({ status: 'processing' }),
    ...
    const stats = {
      ...
      photoRequests: {
        ...
        processing: processingPhotoRequests,
        total: pendingPhotoRequests + processingPhotoRequests,
      },
      revenue: {
...
export const getAlerts = async (
  ...
  try {
    ...
    // 🔥 NEW: Check for pending photo requests (HIGH PRIORITY)
    const pendingPhotoRequests = await PhotoRequest.countDocuments({
      status: 'pending',
    });
    ...
    if (pendingPhotoRequests > 0) {
      alerts.push({
        type: 'info',
        priority: 'high',
        message: `${pendingPhotoRequests} photo request(s) awaiting processing`,
        action: 'Review photo requests',
        link: '/admin/photo-requests?status=pending',
      });
    }

    // 🔥 NEW: Check for photo requests in progress
    const processingPhotoRequests = await PhotoRequest.countDocuments({
      status: 'processing',
    });

    if (processingPhotoRequests > 0) {
      alerts.push({
        type: 'info',
        priority: 'medium',
        message: `${processingPhotoRequests} photo request(s) in progress`,
        action: 'View in-progress requests',
        link: '/admin/photo-requests?status=processing',
      });
    }
